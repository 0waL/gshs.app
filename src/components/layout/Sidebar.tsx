...
export async function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col w-64 fixed left-0 top-0 h-full border-r backdrop-blur-xl z-50"
      ...
      <div className="flex-1 flex flex-col gap-1 px-4 overflow-y-auto">
        ...
        {(user?.role === "BROADCAST" || user?.role === "ADMIN") && (
          <Link href="/music" className="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-colors"
            style={{
        ...
        {user?.role === "ADMIN" && (
          <Link href="/admin" className="flex items-center gap-3 px-4 py-2.5 rounded-xl transition-colors"
            style={{
              color: "var(--foreground)",
              backgroundColor: "transparent",
            }}
      ...
      <div className="p-4 border-t mt-auto flex-shrink-0" style={{ borderColor: "var(--border)" }}>
        {user ? (
          <div className="flex flex-col gap-2">
            <Link href="/me" className="flex items-center gap-3 px-4 py-3 rounded-xl transition-colors" style={{ backgroundColor: "var(--surface-2)" }}>
              <div className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs" style={{ backgroundColor: "var(--accent)", color: "var(--brand-sub)" }}>
                {user.name?.[0] || "U"}
              </div>
